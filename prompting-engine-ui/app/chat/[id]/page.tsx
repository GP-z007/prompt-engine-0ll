  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ messages: newChatHistory, ip: id }), 
      });
  ...
  return (
    <div className="flex flex-col h-screen bg-zinc-950 text-zinc-100 font-sans">
      <header className="px-6 py-4 border-b border-zinc-800 flex items-center justify-between bg-zinc-950/80 backdrop-blur-md sticky top-0 z-10">
        <div>
          <h1 className="font-semibold text-zinc-100">Prompting Engine</h1>
          <p className="text-xs text-zinc-400 mt-1">IP: {id}</p>
        </div>
        ...
        <div className="max-w-3xl mx-auto">
          <form onSubmit={handleSubmit} className="relative flex items-center">
            <input
              ...
              disabled={isLoading}
              placeholder="E.g., Make me a prompt for writing a cold sales email..."
